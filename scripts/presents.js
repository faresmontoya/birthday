export const presents = [
  {
    message: 'Find me bad bunny glasses',
    image: 'images/bb-glasses.JPG'
  },
  {
    message: 'Let me hear you say OV HO!',
    image: 'images/kendrick.jpeg'
  },
  {
    message: 'Web hosting is expensive, donate to support the cause',
    image: 'images/venmo.jpg'
  },
  {
    message: "Let's split the G my G",
    image: 'images/guiness.jpg'
  },
  {
    message: 'Say "Feliz Cumpleaños mi amigo Mexicano!!!"',
    image: ''
  },
  {
    message: 'Where ma hug at?',
    image: 'images/sus-cat.jpg'
  },
  {
    message: '...Drunk cigs??',
    image: 'images/side-eye.jpg'
  },
  {
    message: 'Help me convince Jonny to finish his beer (be as obnoxious as possible)',
    image: 'images/jonny.jpg'
  },
  {
    message: 'Help me convince Pcon to finish his beer while being overly touchy (he likes that)',
    image: 'images/pcon.jpg'
  },
  {
    message: 'Give me your entire 401K retirement fund',
    image: 'images/sus-cat.jpg'
  },
  {
    message: 
    `DANCE BATTLE!!:
    Let's trade off Michael Jackson dance moves with their respective HEE HEEs`,
    image: 'images/michael-jackson.JPG'
  },
  {
    message: 'Join me in aura farming',
    image: 'images/aura-farm.jpeg'
  },
  {
    message: 'Give me your best life advice',
    image: 'dog-suit.jpg'
  },
  {
    message: 'Help me convince JB to finish his beer',
    image: 'images/jb.jpg'
  },
  {
    message: 'Help me convince Wilber to finish his beer',
    image: 'images/wilber.jpg'
  },
  {
    message: 'Tell me your deepest darkest secret',
    image: 'images/side-eye.jpg'
  },
  {
    message: 'Tell me the stupidest shit you have done, but in Spanish',
    image: 'dog-suit.jpg'
  },
  {
    message: 'Take a look at my dog......... Compliment him',
    image: 'images/ovy.JPG'
  },
  {
    message: "Let's go find my brother and convince him to finish his beer (say 'CULO SI NO' if he refuses)",
    image: 'images/brother.JPG'
  },
  {
    message: "Let's send my cousin a voice message saying something borderline (or explicitly) sus",
    image: 'images/cousin.jpg'
  },
  {
    message: 'Tell me a funny joke or story, if I laugh I have to finish my beer',
    image: 'dog-suit.jpg'
  },
  {
    message: 'Guess my nationality: guess wrong you finish your beer, guess right I finish mine ',
    image: 'dog-suit.jpg'
  },
  {
    message: "Guess which bar I have been kicked out of: guess wrong you finish your beer, guess right I finish mine (hint: it's in old town)",
    image: 'dog-suit.jpg'
  },
  {
    message: `Last to finish the math problem finishes their beer: ${(Math.random()*100)`+`(Math.random()*100)`*`(Math.random()*100)}`,
    image: 'dog-suit.jpg'
  },
  {
    message: "WHO'S CALLING THAT SHIT? (dance) Who's callin' that shit? Who's callin' that shit? Who's callin' that shit? Who's callin' that shit?Is it Stacy? Is it Becky? Is it Keisha? Is it Ellie? Was it Dani? Is it PARTY? Where's the function? Send the addy ",
    image: 'drake.jpg'
  },
  {
    message: "Send my sister a nice voice message",
    image: 'sister.jpg'
  },
  {
    message: "We totally should not chug our beers right now......",
    image: 'side-eye.jpg'
  },

]